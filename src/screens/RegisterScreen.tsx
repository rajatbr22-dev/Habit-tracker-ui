...
const RegisterScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const { alertProps, error: showError } = useAlert();
  ...
  const registerMutations = useMutation({
    ...
    onError: (error) => {
      ...
      showError(error.message, { title: 'Registration Failed' });
  ...
  return (
    <KeyboardAvoidingView
      ...
      <ScrollView
        ...
        {/* ── CTA ── */}
        <Pressable
          disabled={registerMutations.isPending}
          style={({ pressed }) => [
            styles.cta,
            SHADOWS.md,
            {
              backgroundColor: registerMutations.isPending
                ? '#ccc'
                : pressed
                  ? BRAND_COLORS.primaryDark
                  : BRAND_COLORS.primary,
              opacity: registerMutations.isPending ? 0.9 : 1,
            },
          ]}
          onPress={handleSubmit(onSubmit)}>
          <View style={styles.ctaContent}>
            <Text 
              style={[typography.button, { fontWeight: '800', color: '#FFF' }]}
            >
              {registerMutations.isPending ? 'Signing Up...' : 'Sign Up'}
            </Text>
            {!registerMutations.isPending && (
